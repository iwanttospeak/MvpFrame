...
import com.app.camera.lisenter.CaptureListener;
import com.app.camera.util.CheckPermission;
import com.app.camera.util.LogUtil;

import static com.app.camera.JCameraView.BUTTON_STATE_BOTH;
import static com.app.camera.JCameraView.BUTTON_STATE_ONLY_CAPTURE;
import static com.app.camera.JCameraView.BUTTON_STATE_ONLY_RECORDER;
...
public class CaptureButton extends View {
    ...
    private int progress_color = 0xEE0091EA;            //进度条颜色
    private int outside_color = 0x99FFFFFF;             //外圆背景色
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_inside_radius = button_radius * 0.82f;

        strokeWidth = button_outside_radius - button_inside_radius;
        outside_add_size = size / 10;
        inside_reduce_size = size / 10;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setColor(outside_color); //外圆（半透明白色）
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if ((button_state == BUTTON_STATE_ONLY_RECORDER || button_state == BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 500);    //同时延长300启动长按后处理的逻辑Runnable
                break;
    ...
    private class LongPressRunnable implements Runnable {
        ...
        public void run() {
            ...
            //启动按钮动画，外圆变大，内圆缩小 -- 同时扩大
            startRecordAnimation(
                    button_outside_radius,
                    button_outside_radius + outside_add_size,
                    button_inside_radius,
                    button_inside_radius + outside_add_size
