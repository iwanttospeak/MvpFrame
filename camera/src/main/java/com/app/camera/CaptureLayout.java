...
import com.app.camera.lisenter.ReturnListener;
...
public class CaptureLayout extends FrameLayout {
    ...
    private TextView txt_time;               //时间文本
    ...
    public void startTypeBtnAnimator(long time) {
        //拍照录制结果后的动画
        if (this.iconLeft != 0)
...
//        btn_capture.setVisibility(GONE);
        if (time !=0){
            txt_time.setText(FileUtil.getDurationTime((int) time) + "");
        }
    ...
    private void initView() {
        ...
        time_param.gravity = Gravity.CENTER;
//        time_param.setMargins(0, 0, 0, 0);
        txt_time.setTextColor(0xff2196F3);
        ...
        //取消按钮
        btn_cancel = new TypeButton(getContext(), TypeButton.TYPE_CANCEL, button_size/4*3);
        ...
        btn_cancel_param.setMargins((layout_width / 6) - button_size / 2, 0, 0, 0);
...
//                resetCaptureLayout();
        ...
        //确认按钮
        btn_confirm = new TypeButton(getContext(), TypeButton.TYPE_CONFIRM, button_size/4*3);
        ...
        btn_confirm_param.setMargins(0, 0, (layout_width / 6) - button_size / 2, 0);
...
//                resetCaptureLayout();
     ...
     * 对外提供的API                      *
     **************************************************/
    //是否是第一次预览，防止 CameraInterface 崩溃
    private boolean isFirstPreview = true;
    public void setCaptureButtonState(int visibility){
        if (isFirstPreview){
            btn_capture.setVisibility(visibility);
            txt_tip.setVisibility(visibility);
