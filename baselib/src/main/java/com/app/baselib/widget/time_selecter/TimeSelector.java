package com.app.baselib.widget.time_selecter;
...
import com.app.baselib.Utils.DateUtil;
...
/**
 * 时间选择器，支持两种时间显示模式，年月日 和 年月日时分
 * Created by liuli on 2015/11/27.
 */
public class TimeSelector {

    public interface ResultHandler {
        void handleTimeData(String data, long timeMills);
    }
    //不同的显示样式，年月日 和 年月日时分
    ...
    private int startYear, startMonth, startDay, startHour, startMininute, endYear, endMonth, endDay,
            endHour, endMininute, minute_workStart, minute_workEnd, hour_workStart, hour_workEnd;
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            seletorDialog = new Dialog(context, R.style.base_dialog_style);
            seletorDialog.setCancelable(true);//设置点击区域外消失
            seletorDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            seletorDialog.setContentView(R.layout.base_dialog_data_selector);
    ...
    private void initView() {
        ...
        year_pv = (PickerView) seletorDialog.findViewById(R.id.base_year_pv);
        month_pv = (PickerView) seletorDialog.findViewById(R.id.base_month_pv);
        day_pv = (PickerView) seletorDialog.findViewById(R.id.base_day_pv);
        hour_pv = (PickerView) seletorDialog.findViewById(R.id.base_hour_pv);
        minute_pv = (PickerView) seletorDialog.findViewById(R.id.base_minute_pv);
        tv_cancle = (ImageView) seletorDialog.findViewById(R.id.base_tv_cancle);
        tv_select = (TextView) seletorDialog.findViewById(R.id.base_tv_select);
        tv_title = (TextView) seletorDialog.findViewById(R.id.base_tv_title);
        hour_text = (TextView) seletorDialog.findViewById(R.id.base_hour_text);
        minute_text = (TextView) seletorDialog.findViewById(R.id.base_minute_text);

        tv_cancle.setOnClickListener(view -> seletorDialog.dismiss());
        tv_select.setOnClickListener(view -> {
            String data = DateUtil.format(selectedCalender.getTime(),FORMAT_STR);
            long timeMills = selectedCalender.getTimeInMillis();
            handler.handleTimeData(data,timeMills);
    ...
    private boolean excuteWorkTime() {
        boolean res = true;
        if (!StringUtils.isEmpty(workStart_str) && !StringUtils.isEmpty(workEnd_str)) {
    ...
    private void dayChange() {
        ...
        day_pv.postDelayed(this::hourChange, CHANGEDELAY);
    }
    /**小时变化*/
